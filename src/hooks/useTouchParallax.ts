...
export function useTouchParallax(
  ...
  useEffect(() => {
    ...
    console.log('[TouchParallax] Setting up', { mode, isMobile, speed })
...
export function useDragParallax(
  ...
  useEffect(() => {
    ...
    return () => {
      container.removeEventListener('mousedown', (e: MouseEvent) => handleStart(e.clientX, e.clientY))
      container.removeEventListener('touchstart', (e: TouchEvent) => {
        if (e.touches.length === 1) {
          handleStart(e.touches[0].clientX, e.touches[0].clientY)
        }
      })
