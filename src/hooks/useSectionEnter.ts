...
export function useSectionEnter(
  ...
  useGSAP(() => {
    ...
    console.log('[useSectionEnter] Setting up for section')

    const elements = section.querySelectorAll('.reveal')
    console.log('[useSectionEnter] Found elements:', elements.length)
    ...
    if (elements.length > 0) {
      ...
      gsap.to(elements, {
        ...
        scrollTrigger: {
          ...
          once: true,
          onEnter: () => {
            console.log('[useSectionEnter] Section entered!')
          },
        },
      })
    }
