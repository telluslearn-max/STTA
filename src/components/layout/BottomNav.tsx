...
export function BottomNav({ onMenuOpen }: BottomNavProps) {
  ...
  const navRef = useCallback((node: HTMLDivElement) => {
    if (node) {
      gsap.fromTo(node,
        { y: 100 },
        { y: 0, duration: 0.5, ease: 'power3.out', delay: 0.5 }
      )
    }
  }, [])
  ...
  const navItems: NavItem[] = [
    ...
    { id: 'menu', icon: <MenuIcon />, label: 'Menu', action: onMenuOpen },
  ]
...
export function MobileMenu({ isOpen, onClose }: MobileMenuProps) {
  ...
  return (
    ...
    >
      ...
      <div ref={menuRef} style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
